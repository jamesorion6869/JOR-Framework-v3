...
For sensor‑only cases (no human witnesses), C is set to 0.30 by convention, 
treating the camera/system as a minimal weak witness; for cases with human witnesses, 
C is scored via the standard rubric.
"""
...
def run():
    ...
    while True:
        ...
        if yes_no("Skip full scoring and enter SOP/NHP directly?"):
            C = E = P_raw = flight_mod_val = P_final = 0
            SOP_manual = get_float_input("Enter SOP (0.0 - 1.0): ", 0.0, 1.0)
            NHP_manual = get_float_input("Enter NHP (0.0 - 1.0): ", 0.0, 1.0)
            SOP, NHP, posterior, pnh, ph = calculate_posterior(SOP_manual, NHP_manual)
        else:
            # NEW: human witness vs camera/system-only
            has_human_witness = yes_no(
                "Is there a human witness that should be scored? "
                "(No = camera/system only; C will default to 0.30)"
            )

            if has_human_witness:
                C = score_factor(
                    C_base_scores, C_modifiers, C_hard_caps, "Witness Credibility"
                )
            else:
                # Camera/system as minimal weak witness per JOR default
                C = 0.30

            E = score_factor(
                E_base_scores, E_modifiers, E_hard_caps, "Environmental Conditions"
            )
            P_raw = score_factor(
                P_base_scores, P_modifiers, P_hard_caps, "Physical Evidence"
            )

            flight_category = choose_flight_category(
                "Select Flight Behavior Classification:",
                Flight_modifiers,
                Flight_descriptors,
            )
